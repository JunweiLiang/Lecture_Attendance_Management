...
class User extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('isSuper, userLevel', 'numerical', 'integerOnly'=>true),
			array('userName, userPw', 'length', 'max'=>64),
			array('isUM', 'length', 'max'=>1),
			array('nickName', 'length', 'max'=>256),
			array('intro', 'length', 'max'=>512),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('userId, userName, userPw, userRegTime, isUM, nickName, intro, isSuper, userLevel', 'safe', 'on'=>'search'),
		);
	}
