	...
	$.post("<?php echo Yii::app()->baseUrl;?>/index.php/user/login",data,function(result){
		...
		else//登录出错，刷新验证码
		{
			if(result.showVarify == 1)
			{
				...
				{
					$("div.loginModal > div.body > div.varify > img.varifyImg").prop("src","<?php echo Yii::app()->baseUrl;?>/index.php/checkIn/vcode");
				}
...
$(document).delegate("div.loginModal > div.body > div.line > img.varifyImg","click",function(){
	$(this).prop("src","<?php echo Yii::app()->baseUrl;?>/index.php/checkIn/vcode?freash="+Math.random());
});
							//进入页面用户名获取焦点
	...
	<div class="loginModal">
		<div class="header">
			...
			-->
			<div class="title title1">Colloquium CheckIn Admin</div>
		</div>
		<div class="body">
			...
			<?php if(!isset(Yii::app()->session['pwWrong']) || (Yii::app()->session['pwWrong'] < 3)){ ?>style="display:none"<?php } ?>>
				...
				<?php if(!isset(Yii::app()->session['pwWrong']) || (Yii::app()->session['pwWrong'] < 3)){ ?>src=""<?php }else{ ?> src="<?php echo Yii::app()->baseUrl;?>/index.php/checkIn/vcode"<?php } ?>></img>
				<!--<a href="#" class="findPw" target="_self">忘记密码？</a>-->
			
				<?php /*
					//cipher method
					<img class='varifyImg' title="图片看不见请刷新页面" alt="请刷新页面" src="<?php echo Yii::app()->baseUrl;?>/Vcode.php?num=<?php echo $code;?>"></img>
					<input type="hidden" id='code' value="<?php echo $encryptedCode;?>"></input>
				*/
				?>
			</div>
